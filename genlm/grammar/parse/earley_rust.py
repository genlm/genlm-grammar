"""
Thin wrapper that exposes the Rust Earley parser with the same interface
as the Python Earley class.  Grammar preprocessing stays in Python;
the hot path (next_column, next_token_weights) runs in Rust.
"""
...
class EarleyRust:
    """
    Drop-in replacement for the Python Earley class that delegates
    ...
    def __init__(self, cfg):
        ...
        # Nonterminals are already integers (from renumber).
        # Assign terminal IDs above max nonterminal.
        ...
        # Encode first_Ys as u32 using the unified encoding
        first_ys_u32 = []
        is_terminal_flags = []
    ...
    def __call__(self, x):
        """Parse token sequence x, return its weight."""
